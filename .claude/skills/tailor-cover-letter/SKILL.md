...
Use the `google-drive-manager` skill to search for a cover letter template in Google Drive. Search for files with "cover letter" in the name. Present any matches to the user so they can confirm which template to use. Export the confirmed file as plain text.

If the skill finds nothing or the user prefers not to use Drive, ask them to paste their cover letter template directly into the chat.
...
If yes:
1. Write the tailored cover letter to a local temp file (e.g. `cover_letter_tailored.txt`).
2. Run `pandoc -o cover_letter_tailored.docx cover_letter_tailored.txt` to convert it to DOCX.
3. Use the `google-drive-manager` skill to upload `cover_letter_tailored.docx` with the name `"[Company] — [Job Title] Cover Letter"`.
